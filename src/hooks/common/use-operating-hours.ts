import type { ClinicOperatingHours, OperatingHours } from '@/types';
...
export const useOperatingHours = (): OperatingHoursHookReturn => {
  ...
  const convertApiToForm = useMemo(
    () =>
      (apiHours?: ClinicOperatingHours[]): Record<string, OperatingHours> => {
  ...
  const convertFormToApi = useMemo(
    () =>
      (formHours: Record<string, OperatingHours>): ClinicOperatingHours[] => {
  ...
  const validateOperatingHours = useMemo(
    () =>
      (hours: Record<string, OperatingHours>): boolean => {
